...
const DisplayEditOffice = () => {
  ...
  const [officeDepartments, setOfficeDepartments] = useState([]);
  ...
  const getDepartData = async () =>{
    const token = localStorage.getItem('token');
    try{
      await axios.get(`${baseUrl}/department/`,{
        headers: {Authorization: `Token ${token}`},
      });
    } catch (error){
      console.error("Error getting the department names: ", error);
    }
  }

  const handleShowInfo = (office) => {
    setOfficeDepartments(office.departments || []);
    ...
    <div className="container mx-auto p-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="rounded-lg shadow-xl w-96 bg-white">
            ...
            <div className="w-[90%] mx-auto">
              {showAddDepartment && <AddDepartOfOffice officeId={showInfoOffice.id} />}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="overflow-x-auto w-full md:w-[90%] mx-auto flex justify-end">
        <table className="w-full bg-white border border-gray-300">
          <thead>
            <tr className="bg-[#3F84E5] text-white">
              <th className="px-4 py-2 border-none">Currently Available Offices</th>
            </tr>
          </thead>
          <tbody>
            {officeData.map((office) => (
              <tr key={office.id} className="border border-b border-gray-300">
                <td className="px-4 py-4 border-none flex items-center justify-between">
                  <div>
