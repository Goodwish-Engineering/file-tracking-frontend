...
const NotificationsTab = ({ onToggleStarred, onNotificationRead }) => {
  ...
  }, [notifications, searchQuery, filterType, sortOrder]);
  ...
  const applyFiltersAndSearch = () => {
    ...
    if (searchQuery.trim()) {
      ...
      results = results.filter(notification => 
        ...
        (notification.related_file?.file_number && notification.related_file.file_number.toLowerCase().includes(query)) ||
        (notification.related_file?.subject && notification.related_file.subject.toLowerCase().includes(query)) ||
        (notification.related_file?.present_by?.first_name && notification.related_file.present_by.first_name.toLowerCase().includes(query)) ||
        (notification.related_file?.present_by?.last_name && notification.related_file.present_by.last_name.toLowerCase().includes(query))
      );
    }
    ...
    <div className="max-w-full mx-auto pb-6">
        ...
        <div className="flex flex-wrap gap-3 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`inline-flex items-center justify-center p-2 border ${showFilters ? 'bg-[#E68332] text-white border-[#E68332]' : 'border-gray-300 text-gray-700 hover:bg-gray-50'} rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#E68332]`}
              title="फिल्टरहरू"
            >
            ...
            <button
              ...
              title={sortOrder === "newest" ? "नयाँ देखि पुरानो" : "पुरानो देखि नयाँ"}
            >
        ...
        {showFilters && (
          <div className="mt-3 p-3 bg-gray-50 border border-gray-200 rounded-lg animate-slideDown">
            <div className="flex flex-wrap gap-2">
              ...
              >
                <MdCheck className={filterType === "all" ? "opacity-100" : "opacity-0"} />
                सबै
              </button>
              ...
              >
                <MdCheck className={filterType === "read" ? "opacity-100" : "opacity-0"} />
                पढिएका
              </button>
              ...
              >
                <MdCheck className={filterType === "unread" ? "opacity-100" : "opacity-0"} />
                नपढिएका
              </button>
            </div>
          </div>
      ...
      {filteredNotifications.length === 0 && (
        <div className="flex flex-col items-center justify-center p-8 text-gray-500">
          <MdNotificationsOff className="text-gray-300 text-6xl mb-4" />
          <h3 className="text-xl font-medium mb-2">कुनै सूचना फेला परेन</h3>
          <p className="text-center text-gray-500 max-w-md">
            {filterType !== "all" || searchQuery
              ? "तपाइँको खोज मापदण्ड अनुसार कुनै सूचना फेला परेन। कृपया फिल्टरहरू परिवर्तन गर्नुहोस्।"
              : "तपाइँको इनबक्स खाली छ। नयाँ सूचनाहरू यहाँ देखिनेछन्।"
            }
          </p>
          {(filterType !== "all" || searchQuery) && (
            <button
          ...
          return (
            ...
            >
              <div className="flex items-start gap-4">
                {/* User avatar */}
                <div className={`flex-shrink-0 w-10 h-10 rounded-full ${userColor} flex items-center justify-center text-white font-medium`}>
                  {initials}
                </div>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className={`p-1 rounded-full hover:bg-gray-200 transition-colors ${
                          notification.is_starred ? "text-yellow-400" : "text-gray-400"
                        }`}
                      >
                  ...
                  <div className="flex flex-wrap gap-2 mt-1 items-center text-xs text-gray-500">
                    ...
                    {notification?.related_file?.subject && (
                      <div className="flex items-center">
                        ...
                        <span className="truncate max-w-xs">{notification.related_file.subject}</span>
                      </div>
                    )}
                  </div>
                  
                  {/* Message */}
                  <p className={`mt-2 text-sm ${!notification.is_read ? "text-gray-900" : "text-gray-600"}`}>
                  ...
                  <div className="mt-2">
                    <span className="inline-flex items-center text-xs font-medium text-[#E68332]">
                      विवरण हेर्नुहोस् <FaExternalLinkAlt className="ml-1" />
                    </span>
                  </div>
                </div>
              </div>
